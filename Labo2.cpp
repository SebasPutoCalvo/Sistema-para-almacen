	...
	int main(){
       ...
       system("cls");
		...
		case 5:
			...
			//luego2
			cout<<endl;
			cout<<"Productos a ingresar?  ",cin>>productos;
			if(productos<5){
			...
			if(matriz1[i][0]==codigo){
				...
				cout<<nombre[i];//nombre
				gotoxy(45,18+i);
				cin>>cantidades[i];//cantidad
				if (cantidades[i]>matriz1[i][3]){
					gotoxy(45,18+i);
					cout<<"No hay mucha existencia"<<endl;
					Sleep(100);
					validacion=0;
				}
			    matriz1[i][3]=matriz1[i][3]-cantidades[i];
				gotoxy(65,18+i);
				cout<<matriz1[i][2];//precio venta
				 venta=cantidades[i]*matriz1[i][2];
				gotoxy(85,18+i);
				cout<<venta<<endl;
